...
void ComputerLogic(char board1T[][11], char board1F[][11], char board2T[][11], char board2F[][11], int size, bool* win2, int* row, int* col, bool* hit, bool* hit_2, bool* hit_3, bool* kill, int* option, int& rowPr, int& colPr) {
    int rowBU = *row;
    int colBU = *col;
    ...
    *hit = CheckHit(board1T, size, *row, *col);
    ...
    ResultOfHit(*kill, *hit, *row, *col, rowPr, colPr);
    rowPr = *row;
    colPr = *col;
